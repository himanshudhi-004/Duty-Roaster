...
export default function GuardDutyHistory() {
  ...
  useEffect(() => {
    if (selectedGuard?.id) {
      fetchDutyHistory(selectedGuard.id);
      return;
    }

    const storedGuard = localStorage.getItem("selectedGuard");
    if (storedGuard) {
      const guardObj = JSON.parse(storedGuard);
      if (guardObj?.id) {
  ...
  const fetchDutyHistory = async (guardId) => {
    try {
      ...
      console.log("Duty History Error:", err);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div style={styles.container}>
      <div style={styles.title}>Guard Duty History</div>
