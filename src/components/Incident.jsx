...
export default function Incident() {
  ...
  const handleStatusUpdate = async (item, action) => {
    try {
      ...
      await axios.post(`${BASE_URL}/api/duty/accidentupdate`, Accidentreq, {
        headers: { Authorization: `Bearer ${token}` },
      });

      setIncidents((prev) =>
        prev.map((inc) =>
          inc.id === item.id ? { ...inc, req: finalStatus } : inc
        )
      );
  ...
  const filteredData = incidents.filter((item) => {
    ...
    const statusMatch = selectedStatus
      ? item.req?.includes(selectedStatus)
      : true;
            ...
            <table style={styles.table}>
              ...
              <tbody>
                {filteredData.map((item, index) => {
                  const statusStyle =
                    item.req?.includes("accepted")
                      ? styles.statusActive
                      : item.req?.includes("rejected")
